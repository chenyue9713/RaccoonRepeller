...
from utils import get_image_tensor,plot_one_box,Colors, resize_and_pad
...
class EdgeTPUModel:
    ...
    def make_interpreter(self):
        ...
        # self.interpreter = etpu.make_interpreter(self.model_file,experimental_delegates=[tfli.load_delegate('libedgetpu.so.1')])
    ...
    def forward(self, x:np.ndarray, with_nms=True) -> np.ndarray:
        ...
        # Scale output
        y = common.output_tensor(self.interpreter, 0).astype('float32')
        result = y
        if int8:
            result = (y - self.output_zero) * self.output_scale
